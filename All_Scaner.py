...
from Ui_AllScaner import Ui_AllScaner


class AllScaner(QWidget, Ui_AllScaner):
    ...
    def scaner(self):
        t = threading.Thread(target=self.scaner_add)
        t.start()
    ...
    def scaner_add(self):
        ...
        items = self.TableWidget.findItems(ean, QtCore.Qt.MatchExactly)
        if len(items) > 0:
            item = items[0]
            # item.setBackground(QBrush(QColor(0, 255, 0)))
            # item.setForeground(QBrush(QColor(255, 0, 0)))
            row = item.row()
            l = self.TableWidget.columnCount()
            self.TableWidget.setCurrentCell(row, l-1)
            text = self.TableWidget.item(row, l-1).text()
            text = int(text)+1
            item2 = QTableWidgetItem(str(text))
            self.TableWidget.setItem(row, l-1, item2)
